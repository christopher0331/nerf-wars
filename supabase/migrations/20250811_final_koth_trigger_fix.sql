-- Migration: Final KOTH Trigger Fix
-- Removes all references to the uuid column which no longer exists

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS process_rfid_scan_trigger ON rfid_scans;

-- Create a simplified function that uses only station ID, not uuid
...
BEGIN
  ...
  -- First, get the station name from its ID (no more uuid references)
  SELECT name INTO station_name 
  FROM stations 
  WHERE id::TEXT = NEW.station_id
  LIMIT 1;
  
  IF station_name IS NULL THEN
    RAISE LOG 'Could not find station with id: %', NEW.station_id;
    RETURN NEW;
  END IF;
...
DO $$
BEGIN
  RAISE NOTICE 'Final KOTH trigger fix installed - removed all uuid references';
END
$$;
