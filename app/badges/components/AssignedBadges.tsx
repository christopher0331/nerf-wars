...
export default function AssignedBadges({
  badges,
  teams,
  loading,
  onUpdatePlayerName,
  ...
  console.log('[COMPONENT_DEBUG] Rendering AssignedBadges with', assignedBadges.length, 'badges')
  console.log('[COMPONENT_DEBUG] Assigned badge UIDs:', assignedBadges.map(b => b.rfid_uid).join(', '))
  ...
  if (assignedBadges.length === 0) {
    return (
      <div className="bg-gray-50 rounded-lg p-4 text-center text-gray-600">
        No badges have been assigned to teams yet.
      </div>
    )
  }
